...
* **Job description** — upload PDF / DOCX / TXT **or** paste text. There is
  no "preset JD" — every text box is fully editable and you can replace
  the JD at any time and re-score. The "Load demo" buttons are just a
  shortcut to pre-fill both boxes with a sample Data-Scientist pair.
* Each headline metric (Resume Score, Keyword, Semantic, Quality) has
  an inline tooltip and a per-component breakdown table that shows
  exactly how the 0-100 Resume Score was built. The explanations
  generated by ResumeMax are produced directly from the scoring
  components and therefore remain fully traceable to the system's
  computations.

The revision history chart tracks Resume Score across iterations.
